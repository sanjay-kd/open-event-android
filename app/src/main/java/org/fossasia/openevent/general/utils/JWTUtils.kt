...
object JWTUtils {
    ...
    fun decode(token: String): SparseArrayCompat<String> {
    ...
    fun getExpiry(token: String): Long {
        ...
        return java.lang.Long.parseLong(JSONObject(decoded.get(1)).get("exp").toString())
    }

    @Throws(JSONException::class)
    fun getIdentity(token: String): Long {
        val decoded = decode(token)

        return java.lang.Long.parseLong(JSONObject(decoded.get(1)).get("identity").toString())
    }
